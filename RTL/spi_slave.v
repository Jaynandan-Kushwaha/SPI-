...
assign header_next = {header_shift[6:0], mosi};
assign data_next = {data_shift[6:0], mosi};
...
always @(posedge clk or posedge reset) begin
    if (reset) begin
        ...
        if (cs) begin
            ...
            if (cpha == 1'b0) begin
                // CPHA=0: leading edge samples MOSI, trailing edge prepares MISO
                if (sclk != cpol) begin
                    if (bit_count < 5'd8) begin
                        ...
                        if (bit_count == 5'd7) begin
                            ...
                            if (header_next[7]) begin
                                case (header_next[6:0])
                                    7'h00:   tx_shift <= 8'hA5;
                                    7'h01:   tx_shift <= control_reg;
                                    7'h02:   tx_shift <= status_reg;
                                    7'h03:   tx_shift <= data_reg;
                        ...
                        if (bit_count == 5'd15) begin
                            if (!read_write) begin
                                case (reg_addr)
                                    7'h01:   control_reg <= data_next;
                                    7'h03:   data_reg <= data_next;
                                    7'h00:   status_reg[0] <= 1'b1;
                                    7'h02:   status_reg[0] <= 1'b1;
                                    default: status_reg[0] <= 1'b1;
                                endcase
                            end
                        end
                    end
                ...
                // CPHA=1: leading edge prepares MISO, trailing edge samples MOSI
                if (sclk != cpol) begin
                    if (response_loaded) begin
                        miso <= tx_shift[7];
                        tx_shift <= {tx_shift[6:0], 1'b0};
